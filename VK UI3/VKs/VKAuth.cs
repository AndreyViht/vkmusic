...
namespace VK_UI3.VKs
{
    internal class VKAuth :VK
    {
        ...
        {
            var result = await _authCategory.GetAuthCodeAsync("Viht Music VK Player", forceRegenerate);
        ...
        */
        ...
        private async Task AuthPasskeyAsync()
        {
            ...
            if (hResult.Succeeded)
            {
                ...
                if (!hResult.Succeeded)
                {
                    _ = login.DialogMessageShow("Не удалось отменить текущий процесс входа, Viht Music VK не смог отменить текущий процесс входа! Закройте все диалоги входа с ключем и повторите попытку.");
