...
namespace SetupLib.Services
{
    public class InstallationService : IInstallationService
    {
        ...
        private async Task InstallApplicationAsync(string packageUrl, PackageType packageType, bool forceInstall, string? PathInstallZIP = null)
        {
            ...
            else // PackageType.ZIP
            {
                ...
                if (PathInstallZIP == null)
                {
                    ...
                    extractPath = Path.Combine(programFilesPath, "Viht Music VK");
        ...
        private string GetZipInstallCommand(string zipPath, string extractPath)
        {
            ...
            sb.AppendLine("# Закрытие процессов Viht Music VK перед установкой");
            sb.AppendLine("$processNames = @('Viht Music VK', 'VK_UI3')");
            ...
            sb.AppendLine("# Поиск исполняемого файла Viht Music VK.exe в целевой директории");
            sb.AppendLine("$exePath = Get-ChildItem -Path $targetDir -Recurse -Filter \"Viht Music VK.exe\" | Select-Object -First 1");
            sb.AppendLine("if (-not $exePath) {");
            sb.AppendLine("    # Если Viht Music VK.exe не найден, ищем любое другое .exe");
